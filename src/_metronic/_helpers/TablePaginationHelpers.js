...
export function PleaseWaitMessage({ entities, intl }) {
  return <>{entities === null && <div>{intl.formatMessage({id: 'PLEASE_WAIT'})}...</div>}</>;
}

export function NoRecordsFoundMessage({ entities, intl }) {
  const customersList = entities === null ? [] : entities;
  return (
    <>
      {customersList.length === 0 && entities !== null && (
        <div>{intl.formatMessage({id: 'NO_RECORD'})}</div>
      )}
    </>
  );
}
